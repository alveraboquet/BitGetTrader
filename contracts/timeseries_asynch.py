...
class TimeSeriesAsynch:
    ...
    async def candles_async(self, session, path):
        ...
        sign = utils.sign(utils.pre_hash(timestamp, c.GET, path, ""), self.api.API_SECRET_KEY)
        header = utils.get_header(self.api.API_KEY, str(sign), str(timestamp), self.api.PASSPHRASE)
    ...
    async def getSeries(self, symbol, granularity, startTime='', endTime='',override=False):
        ...
        async with client as session:
            ...
            for x in range(0,iterations):
                ...
                path = path +"&startTime=" +str(startTime) +"&endTime=" +str(endTime)
                tempSeries.append(asyncio.ensure_future(self.candles_async(session,path)))
                startTime = endTime
